...
public class GroundGenerator : MonoBehaviour {
    ...
    private void Awake() {
        ...
        Instance = this;
        backgroundGenerator = GetComponentInParent<GridManager>().GetBackgroundGenerator();
    }
    ...
    private void GridManager_HasLoadSavedFile(object sender, EventArgs e) {
        InitializeGridSaved();
    }
    ...
    private void GroundGenerator_BackgroundCreated(object sender, EventArgs e) {
        ...
        if (GridManager.Instance.HasLoadSavedFile()) {
            // If has load saved file
    ...
    private void InitializeGridSaved() {

        int currentIndex = 0;
        List<GridNode> gridNodeList = GridManager.Instance.GetGridMap();

        gridMap = new Grid[groundSO.mapWidth, groundSO.mapHeight];


        foreach (GridNode gridNode in gridNodeList) {
            ...
            if (gridNode.GetGridID() == IGridNode.Grid.Fence) {
                ...
                groundTileMap.SetTile(new Vector3Int(nodePos.x, nodePos.y, 0), groundSO.floorTile);
            }

            if (gridNode.GetGridID() == IGridNode.Grid.Floor || gridNode.GetGridID() == IGridNode.Grid.Path) {
                groundTileMap.SetTile(new Vector3Int(nodePos.x, nodePos.y, 0), groundSO.floorTile);
            }

            if (currentIndex == gridNodeList.Count - 1) {
                // Reached last node
    ...
    private void InitializeGrid() {
        ...
        for (int x = 0; x < groundSO.mapWidth; x++) {
            ...
            for (int y = 0; y < groundSO.mapHeight; y++) {
                ...
                int currentIndex = GridManager.Instance.GetGridMap().Count;

                GridManager.Instance.GetGridMap().Add(new GridNode(new Vector2Int(x, y), IGridNode.Grid.Empty, currentIndex));
            }
    ...
    public void SetGridTileTo(Vector2Int tilePosition, Grid grid) { 
        ...
        switch (grid) {
            case Grid.Path:
                ...
                GridManager.Instance.GetNode(tilePosition).SetGridID(IGridNode.Grid.Path);
            ...
            case Grid.Fence:
                ...
                GridManager.Instance.GetNode(tilePosition).SetGridID(IGridNode.Grid.Fence);
            ...
            case Grid.Floor:
                ...
                GridManager.Instance.GetNode(tilePosition).SetGridID(IGridNode.Grid.Floor);
